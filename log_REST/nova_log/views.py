...
def nova_log_view(request):
    ...
    if p_start_date != "unspecified":
        try:
            start_date = datetime.strptime(p_start_date, "%Y:%m:%d-%H:%M:%S")
        except TypeError:
    ...
    if p_end_date != "unspecified":
        try:
            end_date = datetime.strptime(p_end_date, "%Y:%m:%d-%H:%M:%S")
        except TypeError:
            return HttpResponse(json.dumps({"result": "invalid value"}), status=404)
        except ValueError:
            return HttpResponse(json.dumps({"result": "invalid value"}), status=404)
    if (start_date and end_date and start_date>end_date)or (end_date and end_date>datetime.now()):
        return HttpResponse(json.dumps({"result": "invalid value"}), status=404)
